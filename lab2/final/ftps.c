...
int main(int argc, const char *argv[]){
    ...
    const char          *port = argv[1];            //port number
    ...
    bind(server_s, (struct sockaddr *)&server_addr, sizeof(server_addr));// assigns the address specified by addr to the socket referred to by the file descriptor sockfd.
    listen(server_s, 10);                           // Queue length is 10, listen for socket connections and limit the queue of incoming connections
    
    addr_len  = sizeof(client_addr);
    connect_s = accept(server_s, (struct sockaddr *)&client_addr, &addr_len);   // accept a new connection on a socket
    
    bzero (out_buf, BUF_SIZE);
    strcpy(out_buf, "Test message from server!!");
