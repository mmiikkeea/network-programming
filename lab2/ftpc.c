...
int main(int argc, const char **argv){
    ...
    int         PORT_NUM    = atoi(argv[2]);   // Change the port num from char array to int
    ...
    --------------------------------------------------
    struct hostent *server;//add
    --------------------------------------------------
    
    client_s = socket(AF_INET, SOCK_STREAM, 0);
    
    // Fill-in the client socket's address information and do a connect with the listening server.
    ...
    --------------------------------------------------
    server = gethostbyname(argv[1]);// add
    bcopy((char *)server->h_addr,(char *)&serv_addr.sin_addr.s_addr,server->h_length);//add
    --------------------------------------------------
    ...
    memcpy(&IP_ADDR, &server_addr.sin_addr.s_addr, 4);// coppy from 2 to 1
